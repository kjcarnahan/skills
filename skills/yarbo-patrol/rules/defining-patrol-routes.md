...
- **Name plans with a `patrol-` prefix** in the app (`patrol-perimeter`, `patrol-driveway`). Plan identifiers are what `start_plan` references; a consistent prefix keeps patrol routes separate from real mowing plans so a scheduler never accidentally starts blades-down work.
